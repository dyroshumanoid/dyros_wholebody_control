...
void CQuadraticProgram::EnableMaxCpuTime(const double &_max_cpu_time)
{
...
real_t CQuadraticProgram::returnObjVal() const
{
    // std::cout << _QPprob.getObjVal() << std::endl;
    return(_QPprob.getObjVal());
}
...
int CQuadraticProgram::SolveQPoases(const int &num_max_iter, VectorXd &solv, bool MPC)
{
    ...
    _options.enableRegularisation = BT_TRUE;     
    _options.epsRegularisation = 1e-6;   
    // _options.printLevel = PL_LOW;
    // _options.printLevel = PL_DEBUG_ITER;
    _options.printLevel = PL_NONE;
    _options.enableNZCTests = BT_TRUE;  // When removing an active constraint, check whether the projected Hessian matrix remains full rank.
                                        // If the rank is expected to drop, immediately add another constraint to the active set 
                                        // to maintain full-rank condition and ensure numerical stability.
